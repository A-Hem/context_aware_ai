...
const DevEnvironment = () => {
  ...
  const [activeView, setActiveView] = useState('terminal'); // 'terminal', 'editor', 'files'
  ...
  // Detect mobile and handle screen size changes
  useEffect(() => {
    const checkMobile = () => {
  ...
  const getFileIcon = (fileName) => {
    ...
    const iconProps = { size: isMobile ? 14 : 16, className: "text-gray-400" };
...
- Plan development roadmap`;
  ...
  const renderFileTree = (node, depth = 0, path = '') => {
    ...
    return (
      <div key={`${currentPath}-${depth}`}>
        <div 
          className={`flex items-center py-2 px-2 hover:bg-gray-700 cursor-pointer ${isMobile ? 'text-sm' : 'text-sm'} relative group`}
        ...
        >
          {isFolder ? (
            <>
              {node.expanded ? <ChevronDown size={isMobile ? 12 : 14} /> : <ChevronRight size={isMobile ? 12 : 14} />}
              <Folder size={isMobile ? 14 : 16} className="text-blue-400 mx-1" />
            </>
          ) : (
            <>
              <span className={`${isMobile ? 'w-[12px]' : 'w-[14px]'}`}></span>
              {getFileIcon(node.name)}
              <span className="mx-1"></span>
            </>
          )}
          <span className={`text-gray-300 truncate ${selectedFile?.name === node.name ? 'text-blue-400 font-semibold' : ''}`}>
            {node.name}
          </span>
  ...
  const MobileNavTabs = () => (
    <div className="flex bg-gray-800 border-t border-gray-700">
      <button
        ...
        className={`flex-1 py-3 px-4 text-xs font-medium flex items-center justify-center ${
          activeView === 'terminal' ? 'text-blue-400 border-t-2 border-blue-400 bg-gray-700' : 'text-gray-400'
        }`}
      >
      ...
      <button
        ...
        className={`flex-1 py-3 px-4 text-xs font-medium flex items-center justify-center ${
          activeView === 'files' ? 'text-blue-400 border-t-2 border-blue-400 bg-gray-700' : 'text-gray-400'
        }`}
      >
      ...
      {selectedFile && (
        <button
          ...
          className={`flex-1 py-3 px-4 text-xs font-medium flex items-center justify-center ${
            activeView === 'editor' ? 'text-blue-400 border-t-2 border-blue-400 bg-gray-700' : 'text-gray-400'
          }`}
        >
  ...
  return (
    <div className={`${isMobile ? 'h-screen' : 'h-screen'} bg-gray-900 text-gray-100 flex flex-col`}>
      {/* Header */}
      <div className={`${isMobile ? 'p-3' : 'p-4'} border-b border-gray-700 bg-gray-800 flex-shrink-0`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <Monitor size={isMobile ? 18 : 20} className="mr-2 text-green-400" />
            <span className={`${isMobile ? 'text-base' : 'text-lg'} font-semibold`}>AI Dev Terminal</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className={`px-3 py-1 rounded-full ${isMobile ? 'text-xs' : 'text-xs'} font-medium bg-gray-700 ${agents[currentAgent].color}`}>
              {isMobile ? agents[currentAgent].name.split(' ')[1] : agents[currentAgent].name}
            </div>
            {!isMobile && (
        ...
        <div className="flex-1 flex flex-col">
          ...
          {activeView === 'terminal' && (
            <div className="flex-1 flex flex-col">
              <div 
                ref={terminalRef}
                className={`flex-1 p-3 overflow-y-auto font-mono text-sm ${isKeyboardVisible ? 'pb-2' : ''}`}
              >
                {terminalHistory.map((entry, index) => (
                  <div key={index} className="mb-2">
                    {entry.type === 'user' && (
                      <div className="text-green-400 break-words">{entry.content}</div>
                    )}
                    {entry.type === 'system' && (
                      <div className="text-blue-400 break-words">{entry.content}</div>
                    )}
                    {entry.type === 'ai' && (
                      <div className={`${entry.agent ? agents[entry.agent].color : 'text-purple-400'} break-words`}>
                        {entry.content}
                      </div>
                    )}
                  </div>
              ...
              {/* Mobile Input */}
              <div className={`p-3 border-t border-gray-700 bg-gray-800 flex-shrink-0 ${isKeyboardVisible ? 'pb-1' : ''}`}>
                <div className="flex items-center space-x-2">
              ...
              <div className="p-3 border-b border-gray-700">
                <div className="flex items-center justify-between mb-3">
                  <h2 className="text-base font-semibold text-gray-200 flex items-center">
                    <Code2 size={18} className="mr-2 text-blue-400" />
                    Project Files
                  </h2>
                  <div className="flex items-center space-x-2">
                    <button
                ...
                <div className="flex space-x-2">
                  <button
                    ...
                    className="flex items-center px-3 py-2 bg-blue-600 hover:bg-blue-700 rounded text-xs"
                    title="Upload Files"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center px-3 py-2 bg-purple-600 hover:bg-purple-700 rounded text-xs"
                  ...
                  <div className="mt-3 space-y-2">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        className="px-3 py-2 bg-gray-600 hover:bg-gray-700 rounded text-xs"
                      >
                        Cancel
                      </button>
                    </div>
